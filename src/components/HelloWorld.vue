<template>
  <div class="principal">
    <div class="rating" v-if="!rate">
      ...
      <div class="numbers">
        <p v-for="n in arr" :key="n" @click="rating=n" :class="rating == n ? 'selected' : ''">{{ n }}</p>
      </div>
      <button @click="rate=!rate">Submit</button>
    </div>
    ...
    <div class="rating-done" v-else>
      ...
      <p class="rate">You selected {{ rating }} out of 5</p>
      <h2 class="thanks">Thank you!</h2>
      <p class="text2">We appreciate you taking the time to give a rating. If you ever need more support, 
...
<style>
  ...
  .principal {
    background: hsla(213, 20%, 18%, 0.683);
    padding: 25px;
    border-radius: 17px;
    width: 290px;
  }
  ...
  .star {
    ...
    justify-content: center;
    align-items: center;
    background: hsla(216, 12%, 54%, 0.151);
    border-radius: 50%;
    width: 40px;
    height: 40px;
  }
